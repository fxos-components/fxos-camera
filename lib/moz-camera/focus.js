...
function Focus(camera) {
  ...
  this.onFacesDetected = throttle(this.onFacesDetected.bind(this), 100);
...
Focus.prototype = {
  ...
  set(rect) {
    ...
    if (rect) return this.setPoint(rect);
    debug('focus', rect);
  ...
  setPoint(rect) {
    if (!this.hasPointFocus) return this.set();
  ...
  setState(state) {
    ...
    debug('set state', state, this.focusArea);
    this.state = state;
    this.callbacks.onFocusChanged(state, this.focusArea);
  },
