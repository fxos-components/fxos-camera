...
function Focus(options) {
  ...
  this.handleFaceDetectionEvent = this.handleFaceDetectionEvent.bind(this);
}

Focus.prototype = {
  configure(mozCamera, mode) {
    ...
    if (this.continuousAutoFocus) {
      if (mode === 'picture') focusMode = 'continuous-picture';
      else if (mode === 'video') focusMode = 'continuous-video';
    }
  ...
  getMode() {
    return this.suspendedMode || this.mozCamera.focusMode;
  },
  ...
  configureFocusModes(mode) {
    ...
    this.faceDetection = faceDetectionUserEnabled
      && faceDetectionSupported
      && mode === 'picture';

    on(this.mozCamera, 'focus', this.onAutoFocusStateChange);
  },

  startFaceDetection() {
    if (!this.faceDetection) return;
    on(this.mozCamera, 'facesdetected', this.handleFaceDetectionEvent);
  ...
  stopFaceDetection() {
    ...
    if (this.mozCamera.stopFaceDetection) {
      off(this.mozCamera, 'facesdetected', this.handleFaceDetectionEvent);
  ...
  onAutoFocusStateChange(e) {
    var state = e.newState;
    if (state === 'unfocused') state = 'fail';
    this.updateFocusState(state);
  },

  // NO OP by default
  onAutoFocusChanged(state) {},
