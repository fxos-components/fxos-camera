 ...
 * @type {Funciton}
 */
var debug = 1 ? (...args) => console.log('[Video]', ...args) : () => {};
...
Video.prototype = {
  ...
  start() {
    return hasSpace(this[key].storage, minRecordSpace)
      .then(result => {
...
function hasSpace(deviceStorage, required) {
  return new Promise((resolve, reject) => {
    debug('get free storage space');

    var req = deviceStorage.freeSpace();
    req.onerror = reject;
