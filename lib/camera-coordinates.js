 ...
 *  @public
 */
function faceToPixels({bounds, viewport, sensorAngle, mirrored}) {
  // The orientation of the screen and the sensor might not match.
  // We rotate the bounds so it matches screen coordinates orientation
  if (sensorAngle) bounds = rotateArea(bounds, sensorAngle);
  if (mirrored) bounds = mirrorAreaCamera(bounds);
  return areaToPixels(bounds, viewport);
}
...
})})(((n,w)=>{return(typeof define)[0]=='f'&&define.amd?define:(typeof module)[0]=='o'?c =>{c(require,exports,module)}:c=>{var m={exports:{}},r=n=>w[n];w[n]=c(r,m.exports,m)||m.exports;};})('./camera-coordinates',this));/*jshint ignore:line*/